<x-app-layout>
    ...
    <div class="flex justify-end p-6">
        <a href="{{ route('solicitudes.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
            Nueva Solicitud
        </a>
    </div>
    {{--filtros--}}
    <form method="GET" action="{{ route('solicitudes.index') }}" class="flex flex-wrap gap-4 mb-6">
        ...
        <a href="{{ route('solicitudes.index') }}" class="text-gray-600 hover:underline">Limpiar</a>
    </form>

    {{--ordeanamiento--}}
    <form method="GET" action="{{ route('solicitudes.index') }}" id="ordenForm" class="mb-6">
                ...
                @forelse($solicitudes as $solicitud)
                    <tr class="border-t hover:bg-gray-50">
                        ...
                        <td class="p-2 space-x-2">
                            <a href="{{ route('solicitudes.edit', $solicitud) }}" class="text-indigo-600 hover:underline">Editar</a>
                            <form action="{{ route('solicitudes.destroy', $solicitud) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
